 ...
 * 
 * This script tests the MongoDB connection and reports the status.
 * It will show whether MongoDB is connected or if it's using the fallback.
 */
...
async function testMongoDBConnection() {
  ...
  log('║                                                              ║');
  log('║         Testing MongoDB Connectivity & Status               ║');
  log('╚══════════════════════════════════════════════════════════════╝');
  log(`${colors.reset}`);
...
async function testMongoDBSpecificFeatures() {
  log(`\n🎯 Testing MongoDB-Specific Features:`, 'cyan');
  
  const result = await testMongoDBConnection();
  
  if (result.connected) {
    log(`\n📋 MongoDB Features Available:`, 'green');
    log('• Persistent message storage', 'green');
    log('• Advanced querying capabilities', 'green');
    log('• Indexing for performance', 'green');
    log('• Aggregation pipelines', 'green');
    log('• Replication support', 'green');
    log('• Sharding for scalability', 'green');
    
    log(`\n📊 Performance Benefits:`, 'green');
    log('• Faster message retrieval', 'green');
    log('• Better query performance', 'green');
    log('• Support for large datasets', 'green');
    log('• Real-time analytics capabilities', 'green');
    
  } else {
    log(`\n📋 Using In-Memory Fallback:`, 'yellow');
    log('• Messages stored in memory', 'yellow');
    log('• Limited to available RAM', 'yellow');
    log('• No persistence across restarts', 'yellow');
    log('• Suitable for development/testing', 'yellow');
  }
...
async function checkMongoDBInstallation() {
  log(`\n🔍 Checking MongoDB Installation:`, 'cyan');
  ...
  try {
    ...
    try {
      ...
      if (stdout.trim()) {
        logSuccess('MongoDB daemon (mongod) is running');
      }
    } catch {
      logWarning('MongoDB daemon (mongod) is not running locally');
    }
    
    // Check MongoDB version if available
    try {
      const { stdout } = await execAsync('mongod --version');
      const versionMatch = stdout.match(/db version v(\d+\.\d+\.\d+)/);
      if (versionMatch) {
        logSuccess(`MongoDB version: ${versionMatch[1]} installed`);
      }
    } catch {
      logInfo('MongoDB CLI tools not found locally');
    }
    
  } catch (error) {
    logInfo('Could not check local MongoDB installation');
  }
}
...
async function runMongoDBTests() {
  ...
  await checkMongoDBInstallation();
  
  // Test connection and features
  const result = await testMongoDBSpecificFeatures();
...
export { runMongoDBTests, testMongoDBConnection };